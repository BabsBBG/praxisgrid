    ...
    <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      <section className="aq-hero overflow-hidden p-5 sm:p-8">
        <div className="grid gap-5 md:grid-cols-[1.25fr_.75fr] md:items-end">
          <div>
            ...
            <p className="mt-3 max-w-2xl text-sm font-semibold text-[var(--aq-muted)] sm:text-base">Source-grounded certification practice, capability progress, real project evidence, and technical interview preparation across provider-neutral career paths.</p>
          </div>
          <div className="aq-subtle-panel grid grid-cols-2 gap-2 p-4 md:grid-cols-1 md:gap-3">
            <div className="flex items-center gap-2"><Gauge className="h-5 w-5 shrink-0" /><span className="text-sm font-semibold">One active goal</span></div>
            <div className="flex items-center gap-2"><GraduationCap className="h-5 w-5 shrink-0" /><span className="text-sm font-semibold">Approved assessments</span></div>
            <div className="flex items-center gap-2"><ShieldCheck className="h-5 w-5 shrink-0" /><span className="text-sm font-semibold">Career Lab evidence</span></div>
      ...
      <section className="grid gap-4">
        {certPaths.map((path, index) => {
          ...
          const content = (
            <Card className={`aq-row-card overflow-hidden p-5 sm:p-6 ${disabled ? "opacity-85" : ""}`}>
              <div className="mb-4 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div className="min-w-0">
                  ...
                  {path.transitionMessage ? <p className="mt-3 rounded-md border border-amber-200 bg-amber-50 p-3 text-sm font-semibold text-amber-950 dark:border-amber-300/40 dark:bg-amber-300/10 dark:text-amber-100">{path.transitionMessage}</p> : null}
                </div>
                <div className="aq-metric flex shrink-0 items-center gap-3 sm:min-w-32 sm:flex-col sm:text-center">
                  <div className="text-3xl font-bold">{value}%</div>
                  <p className="text-xs font-bold uppercase tracking-[0.04em] text-[var(--aq-muted)]">Progress</p>
                </div>
              </div>
              <Progress value={value} />
              <div className="mt-4 flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
                <p className="text-sm font-semibold text-slate-500 dark:text-slate-400">{disabled ? "Historical activity preserved; new activation disabled." : path.examFormat}</p>
                <span className="inline-flex items-center gap-2 text-sm font-semibold text-slate-950 dark:text-white">{disabled ? `Continue with ${path.replacementCert ?? "an active path"}` : "Open path"} <ArrowRight className="h-4 w-4" /></span>
              </div>
            </Card>
          );
          return (
            <motion.div key={path.cert} initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.05 }}>
              <Link to={`/cert/${pathFor(disabled ? path.replacementCert ?? "SC-500" : path.cert)}`}>{content}</Link>
            </motion.div>
          );
        })}
      </section>
