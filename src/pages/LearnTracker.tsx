  ...
  return (
    <motion.div initial={{ opacity:0, y:16 }} animate={{ opacity:1, y:0 }} className="space-y-4">
      ...
      {certs.map(cert => {
        ...
        return <Card key={cert}><CardHeader><div><Badge>{cert}</Badge><CardTitle className="mt-2">{docSet?.title ?? `${cert} Learning`}</CardTitle><p className="text-sm font-semibold text-[var(--aq-muted)]">{done}/{resources.length} resources complete</p></div><Badge>{pct}%</Badge></CardHeader><Progress value={pct} /><CardContent><div className="grid gap-3 md:grid-cols-2">{resources.map((r:any)=><div key={r.id} className="aq-row-card p-4"><div className="flex items-start justify-between gap-3"><div><p className="font-semibold">{r.label}</p><p className="text-sm font-semibold text-[var(--aq-muted)]">{r.description ?? r.domain ?? "Official learning resource"}</p></div>{r.type === "video" ? <Film className="h-5 w-5 text-[var(--aq-blue-600)]" /> : <FileText className="h-5 w-5 text-[var(--aq-blue-600)]" />}</div><div className="mt-3 flex gap-2"><Button onClick={() => void toggle(r.id)} variant={completed.includes(r.id) ? "success" : "soft"}>{completed.includes(r.id) ? <CheckCircle2 className="h-4 w-4" /> : null}{completed.includes(r.id) ? "Complete" : "Mark done"}</Button>{r.url ? <Button asChild variant="ghost"><a href={r.url} target="_blank" rel="noreferrer"><ExternalLink className="h-4 w-4" /> Open</a></Button> : null}</div></div>)}</div></CardContent></Card>
      })}
    </motion.div>
  );
}
